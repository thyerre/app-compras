...
import { OrgaoResponse } from '../../../../core/models/planejamento.model';
...
@Component({
  ...
  styleUrl: './orgao-list.component.scss',
})
export class OrgaoListComponent extends BaseListComponent<OrgaoResponse> {
  ...
  loadData(): void {
    ...
    this.planejamentoService.findAllOrgaos(this.filters(), this.pageIndex(), this.pageSize()).subscribe({
      next: page => {
  ...
  onEditItem(item: OrgaoResponse): void {
    this.onEdit(item.id);
  }

  onDeleteItem(item: OrgaoResponse): void {
    if (confirm('Tem certeza que deseja excluir este órgão?')) {
      this.planejamentoService.deleteOrgao(item.id).subscribe({
        next: () => {
          this.snackBar.open('Órgão excluído com sucesso!', 'Fechar', { duration: 3000, horizontalPosition: 'center', verticalPosition: 'top' });
          this.loadData();
        },
      });
    }
